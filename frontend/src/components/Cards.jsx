export default function Cards({ details }) {
  ...
  const { name, description, img, github, linkedIn, twitter } = details;

  return (

    <>
      <div className="card_container">
        <div className="img_renderer" style={{"backgroundImage":`url(${img})`}}>
          {/* <img src={img} alt="profileImage" width={200} height={200}/> */}
        </div>

        <div className="card_desc_container">
          <p style={{ "fontSize": "2rem", "paddingLeft": "2rem", "color": "white" }} >{name}</p>
          <p style={{ "paddingLeft": "2rem" }}> {description} </p>
           ...
           {github ? (
...

            {
              linkedIn ? (
                ...
                <a
              data-v-54e46119=""
              href={linkedIn ? linkedIn:"https://www.linkedin.com/"}
              rel="noopener noreferrer"
              target="_blank"
              className="pl-7 invisible md:visible"
            >
